...
```{r}
# function to compute days in a month: input is year and month
# this is the best I could do without downloading external R libraries
...
for (eachLine in 1:length(myData)) {
  ...
  if (metVar == "PRCP") {
    ...
    for (eachDay in 1:FnDaysInMonth(yrVar, moVar)) {                
      ...
      metVal <- as.numeric(substr(myData[eachLine], dayOffset, dayOffset+4))
      ...
      #write to ouput
      if (qflag == ' ' & sflag != ' ') {
        writeLines(paste(yrVar, moVar, eachDay, metVal, sep = ","), outFile)
      }
    }		
  }        
}
...
plot(yrtot$val ~ yrtot$yr, type="h", main="Annual Precipitation Total (inches), Livermore, CA", ylab="inches/year", xlab="year")
garbage <- dev.off()

#monthly total
montot <- aggregate(val ~ yr + mo, data=prcp, FUN=sum)
png(filename="fig2.png")
boxplot(montot$val ~ montot$mo, range=0, main="Monthly Precipitation Total (inches), Livermore, CA", ylab="inches/month", xlab="calendar month")
garbage <- dev.off()
...
boxplot(raindays$val ~ raindays$mo, range=0, main="Monthly Rainy Days, Livermore, CA", ylab="days/month", xlab="calendar month")
garbage <- dev.off()
